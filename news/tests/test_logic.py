...
# При запуске тестов создаётся пустая база данных, так что проверим, что после отправки запроса число объектов модели Comment останется равным нулю.
...
# После этого проверим, что поля комментария содержат корректную информацию
...
# с комментариями, поэтому к адресу страницы добавлено указание на нужный раздел: #comments.
# Такие указатели называют anchor, «якорь». Ссылка с якорем позволяет не просто сослаться на страницу,
# а прокрутить эту страницу до определённого раздела; в нашем случае — до HTML-тега с атрибутом id="comments".
...
# Важно помнить, что в django.test каждый тест происходит в транзакции, и удаление комментария в одном
# тесте никак не повлияет на другие тесты в этом классе; в начале следующих тестов база данных вновь будет
# содержать один комментарий с исходными значениями, заданными при создании объекта комментария.
...
# После редактирования комментария нужно проверить, изменилось ли его содержимое; для этого нужно сравнить поле text объекта комментария со значением NEW_COMMENT_TEXT.
# Однако в Python-объекте self.comment хранится исходное состояние комментария (то, что было до редактирования); этот объект не обновится при изменении записи в БД.
# при сравнении значений self.comment.text и NEW_COMMENT_TEXT тест провалится. Необходимо обновить объект self.comment; для этого применим метод refresh_from_db().
